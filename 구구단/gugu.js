//구구단만들기


//ceil 은 올림이다. 소수점 올려주는거
//floor은 내림이다. 소수점 없애지는거
var 숫자1 = Math.ceil(Math.random() * 9);
var 숫자2 = Math.ceil(Math.random() * 9);

//구구단 숫자 계산 숫자 두개 필요
var 결과 = 숫자1*숫자2;


var 바디 = document.body;
var 단어 = document.createElement('div');

단어.textContent = String(숫자1) + '곱하기' + String(숫자2) + '는?';
document.body.append(단어);

var 폼 = document.createElement('form');
document.body.append(폼);

var 입력창 = document.createElement('input');
입력창.type = 'number';
폼.append(입력창);

var 버튼 = document.createElement('button');
버튼.textContent = '입력!';
폼.append(버튼);

var 결과창 = document.createElement('div');
document.body.append(결과창);

폼.addEventListener('submit', function 콜백함수 (e) {
  e.preventDefault();
  if (결과 === Number(입력창.value)) {//타입을 숫자로 바꿔주기위해
    결과창.textContent = '딩동댕';
      //맞으면 새로운 질문을 준비해야하기 때문에 이 값을 추가해준다.
    //var 빼고 숫자1 그냥 쓰면 기존에 있는것을 값을 바꿔서 쓴다는 의미
    숫자1 = Math.ceil(Math.random() * 9);
    숫자2 = Math.ceil(Math.random() * 9);
    결과 = 숫자1 * 숫자2;
    단어.textContent = String(숫자1) + '곱하기' + String(숫자2) + '는?';
    입력창.value = '';
    입력창.focus();
  } else {
    결과창.textContent = '땡';
    입력창.value = ''; //입력창 비우기
    입력창.focus(); //고정
  }
});
